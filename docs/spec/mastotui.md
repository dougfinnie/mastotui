...
Given any main view (Login, Timeline, TootDetail, Compose), when the user opens instance info (from Login/Timeline/TootDetail: press `i`; from Compose: press Ctrl+`i` so that plain letters insert into the toot), the client MUST show the current instance URL, whether the user is logged in or browsing anonymously, and options to log out / log in or browse another instance.
...
Given the instance info screen is open and the user presses `b`, the client MUST open a dialog with a text box to enter an instance URL and an option to pick from known instances (e.g. the instance from config where the user has authenticated). (Instance info is opened by pressing `i` from Login, Timeline, or TootDetail, or Ctrl+`i` from Compose; see r[instance.info.dialog].)
